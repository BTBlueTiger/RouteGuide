...
NominatimRessource::NominatimRessource(QObject *parent): AbstractResource(parent)
{
    url->setScheme("http");
...
void NominatimRessource::setSearch(const QString& search)
{
    if(m_search != search)
    {
        m_search = search;
...
void NominatimRessource::searchQ()
{
    if(m_search.isEmpty())
        return;
    ...
    connect(reply, &QNetworkReply::finished, this, &NominatimRessource::networkRequestFinished);
}

void NominatimRessource::networkRequestFinished() {
